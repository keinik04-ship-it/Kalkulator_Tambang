...
def run_streamlit_app():
    ...
    # PERUBAHAN RUMUS: (Debit Max * 1.5) / Kecepatan Pengendapan
    ...
    # Informasi Rumus yang digunakan
    st.info(f"**Info Perhitungan:** Luas Total = (Debit Max {debit_max} × 1.5) / V. Stokes")
    ...
    with tab1:
        ...
        with col_a:
            st.write(f"**Sisi Dasar (Bawah):** {math.sqrt(luas_kol_1):.2f} m")
            st.write(f"**Sisi Permukaan (Atas):** {s_atas_1:.2f} m")
            st.write(f"**Luas Alas per Unit:** {luas_kol_1:.2f} m²")
        with col_b:
    ...
    with tab2:
        if jumlah_kolam > 1:
            col_a, col_b = st.columns(2)
            with col_a:
                st.write(f"**Sisi Dasar (Bawah):** {math.sqrt(luas_sisa):.2f} m")
                st.write(f"**Sisi Permukaan (Atas):** {s_atas_sisa:.2f} m")
    ...
    st.markdown("---")
    st.caption("Volume dihitung menggunakan rumus Frustum (Limas Terpancung) dengan faktor luas 1.5x debit.")
